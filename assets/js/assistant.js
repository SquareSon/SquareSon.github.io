(() => {
  ...
  const copy = {
    zh: {
      ...
      autoTimeBudgetExceeded: '自动快车道在 20 秒内未获得完整回答，已停止继续等待慢速模型。你可以重试、手动选择模型，或使用站内资料搜索。',
      autoRouteUnavailable: '自动快车道中的模型当前均不可用，系统未切入慢速模型。请稍后重试、手动选择模型，或使用站内资料搜索。',
    ...
    en: {
      ...
      autoTimeBudgetExceeded: 'The automatic fast lane did not produce a complete answer within 20 seconds, so it stopped instead of waiting for slower models. Retry, choose a model manually, or use on-page material search.',
      autoRouteUnavailable: 'All models in the automatic fast lane are currently unavailable, so the system did not switch to slower models. Retry later, choose a model manually, or use on-page material search.',
      ...
      autoSource: 'Auto / speed priority',
      autoModel: 'Auto / speed priority',
    },
  }[locale];
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const messages = {
        ...
        auto_time_budget_exhausted: copy.autoTimeBudgetExceeded,
        auto_route_unavailable: copy.autoRouteUnavailable,
