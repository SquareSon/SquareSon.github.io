...
test("public RAG corpus is bounded, attributable, and privacy-safe", () => {
  assert.equal(corpus.length, 321);
  ...
  assert.ok(corpus.some((item) => item.id === "fact-project-pllbj"));
  assert.ok(corpus.some((item) => item.id === "fact-project-pllbj-en"));
  assert.ok(corpus.some((item) => item.id === "fact-project-neural-rrt"));
  assert.ok(corpus.some((item) => item.id === "fact-project-neural-rrt-en"));
